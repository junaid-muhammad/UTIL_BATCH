...
if [ -f "${ROOTFILE}" ]; then
	...
	eval "$REPLAYPATH/hcana -l -q \"SCRIPTS/COIN/CALIBRATION/"$OPT"Cal_Calib_Coin_test.C($RUNNUMBER,$MAXEVENTS)\""
    else
...
REPLAYSCRIPT="${REPLAYPATH}/SCRIPTS/COIN/CALIBRATION/${OPT}Cal_Calib_Coin_test.C"
...
# Note, the PREFIX argument to the hcana script will need to be changed according to the replay script
 if [ $OPT == "HMS" ]; then
     eval "${REPLAYPATH}/hcana -l -q -b 'hcal_calib.cpp+(\"HMS_Cal_Calib_test\", $RUNNUMBER, $MAXEVENTS)'"
     sleep 2
     if [ -f "HMS_Cal_Calib_test_${RUNNUMBER}_${MAXEVENTS}.pdf" ]; then
	 mv "HMS_Cal_Calib_test_"$RUNNUMBER"_"$MAXEVENTS".pdf" "Output/HMS_Cal_Calib_test_"$RUNNUMBER"_"$MAXEVENTS".pdf"
     elif [ ! -f "HMS_Cal_Calib_test_${RUNNUMBER}_${MAXEVENTS}.pdf" ]; then 
	 echo "Calibration output pdf not found! Calibration may have failed!"
     fi
     if [ -f "hcal.param.HMS_Cal_Calib_test_${RUNNUMBER}_${MAXEVENTS}" ]; then
	 mv "hcal.param.HMS_Cal_Calib_test_"$RUNNUMBER"_"$MAXEVENTS "Output/hcal.param.HMS_Cal_Calib_test_"$RUNNUMBER"_"$MAXEVENTS
     elif [ ! -f "hcal.param.HMS_Cal_Calib_test_${RUNNUMBER}_${MAXEVENTS}" ]; then
	 echo "Calibration output param file not found! Calibration may have failed!"
     fi
 fi
 if [ $OPT == "SHMS" ]; then
     eval "${REPLAYPATH}/hcana -l -q -b 'pcal_calib.cpp+(\"SHMS_Cal_Calib_test\", $RUNNUMBER, $MAXEVENTS)'"
     sleep 2
     if [ -f "SHMS_Cal_Cali_testb_${RUNNUMBER}_${MAXEVENTS}.pdf" ]; then
	 mv "SHMS_Cal_Cali_testb_"$RUNNUMBER"_"$MAXEVENTS".pdf" "Output/SHMS_Cal_Calib_test_"$RUNNUMBER"_"$MAXEVENTS".pdf"
     elif [ ! -f "SHMS_Cal_Calib_test_${RUNNUMBER}_${MAXEVENTS}.pdf" ]; then 
	 echo "Calibration output pdf not found! Calibration may have failed!"
     fi
     if [ -f "pcal.param.SHMS_Cal_Calib_test_${RUNNUMBER}_${MAXEVENTS}" ]; then
	 mv "pcal.param.SHMS_Cal_Calib_test_"$RUNNUMBER"_"$MAXEVENTS "Output/pcal.param.SHMS_Cal_Calib_test_"$RUNNUMBER"_"$MAXEVENTS
     elif [ ! -f "pcal.param.SHMS_Cal_Calib_test_${RUNNUMBER}_${MAXEVENTS}" ]; then
	 echo "Calibration output param file not found! Calibration may have failed!"
     fi
 fi
exit 0
