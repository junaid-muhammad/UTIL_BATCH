...
echo "Run type must be one of - Prod - Lumi - LumiCoin - HeePSingHMS - HeePSingSHMS - HeePCoin - pTRIG6 - Case sensitive!"

RUNTYPE=$1
RUNNUMBER=$2
MAXEVENTS=$3
# Need to change these a little, should check whether arguments are good or not REGARDLESS of whether they're blank
if [[ -z "$1" || ! "$RUNTYPE" =~ Prod|Lumi|LumiCoin|HeePSingHMS|HeePSingSHMS|HeePCoin|pTRIG6 ]]; then # Check the 2nd argument was provided and that it's one of the valid options
    echo ""
    echo "I need a valid run type"
    while true; do
	echo ""
	read -p "Please type in a run type from - Prod - Lumi - LumiCoin - HeePSingHMS - HeePSingSHMS - HeePCoin - pTRIG6 - Case sensitive! - or press ctrl-c to exit : " RUNTYPE
	case $RUNTYPE in
	    '');; # If blank, prompt again
	    'Prod'|'Lumi'|'LumiCoin'|'HeePSingHMS'|'HeePSingSHMS'|'HeePCoin'|'pTRIG6') break;; # If a valid option, break the loop and continue
	esac
    done
fi
...
if [[ $RUNTYPE == "Prod" ]]; then
    ...
    eval "$REPLAYPATH/hcana -l -q \"UTIL_PION/scripts/replay/PionLT/FullReplay_PionLT_Luminosity.C($RUNNUMBER,$MAXEVENTS)\""
elif [[ $RUNTYPE == "LumiCoin" ]]; then
    echo "Running luminosity Coin analysis script - ${RUNTYPE}"
    eval "$REPLAYPATH/hcana -l -q \"UTIL_PION/scripts/replay/PionLT/FullReplay_PionLT_Luminosity_Coin.C($RUNNUMBER,$MAXEVENTS)\""
